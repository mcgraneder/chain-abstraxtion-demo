...
describe("Staking", function () {
  ...
  describe("Forwarder", function () {
    it("should be able to deposit through frowarder", async function () {
      ...
      const ALICEabcUnsignedTx3 = await wbtc.populateTransaction.transfer(
        owner.address,
        100
      );
      const ALICEabcSignedTx3 = await sign(ALICEabcUnsignedTx3, wallet1);
      await forwarder.execute(ALICEabcSignedTx3.values, ALICEabcSignedTx3.sig);
      console.log("balance after", Number(await wbtc.balanceOf(owner.address)));
    });
  });
});
